...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      <div className="editable-display flex items-center gap-2 text-2xl font-semibold uppercase">
        <MessageCircle className="h-5 w-5 text-[var(--tk-accent)]" /> Comments
        <span className="text-[var(--tk-muted)]">({all.length})</span>
      </div>

      <form onSubmit={submit} className="mt-6 rounded-sm border border-[var(--tk-line)] bg-[#f4e9f2] p-5">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="Your name (optional)"
          maxLength={60}
          className="h-11 w-full rounded-sm border border-[var(--tk-line)] bg-white px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
          ...
          rows={3}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-sm border border-[var(--tk-line)] bg-white px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-sm bg-[var(--tk-accent)] px-6 py-2.5 text-sm font-bold text-[var(--tk-on-accent)] transition hover:bg-[#8D5F8C] active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid gap-3">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-sm border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5">
