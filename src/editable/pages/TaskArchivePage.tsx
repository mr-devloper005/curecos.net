...
const cardBase = 'group block rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_24px_54px_rgba(107,63,105,0.14)]'
    ...
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)] bg-white">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-2 bg-[#8D5F8C]" />
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-12 sm:py-16 lg:px-8">
            ...
            <h1 className="editable-display mt-4 max-w-3xl text-balance text-[2.4rem] font-semibold uppercase leading-[1.05] sm:text-5xl">
            ...
            <div className="mt-9 flex flex-col gap-4 border-t border-[var(--tk-line)] pt-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-11 appearance-none rounded-sm border border-[var(--tk-line)] bg-[var(--tk-surface)] pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-11 items-center rounded-sm bg-[var(--tk-accent)] px-5 text-sm font-semibold text-[var(--tk-on-accent)] transition hover:bg-[#8D5F8C]">Apply</button>
              </form>
            </div>
          </div>
        </header>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
          ...
          {posts.length ? (
            <nav className="mt-14 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-sm border border-[var(--tk-line)] bg-white px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Previous</Link> : null}
              <span className="rounded-sm border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 font-medium text-[var(--tk-muted)]">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-sm border border-[var(--tk-line)] bg-white px-5 py-2.5 font-medium transition hover:border-[var(--tk-accent)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>
      </main>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <>
      {index === 0 ? (
        <div className="mx-auto w-full max-w-[970px] px-4 py-6">
          <Ads slot="header" showLabel eager className="mx-auto min-h-[90px] w-full" />
        </div>
      ) : null}
      <Link href={href} className="group grid gap-4 border-b border-dashed border-[var(--tk-line)] bg-white px-4 py-5 transition hover:bg-[var(--tk-raised)] sm:grid-cols-[120px_minmax(0,1fr)_72px] sm:items-center">
        <div className="aspect-[4/3] overflow-hidden rounded-sm bg-[var(--tk-raised)]">
          <img src={image} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.03]" />
        </div>
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--tk-accent)]">
            <span>{category}</span>
            <span className="text-[var(--tk-muted)]">· No. {String(index + 1).padStart(2, '0')}</span>
          </div>
          <h2 className="editable-display mt-2 text-2xl font-semibold leading-snug tracking-[-0.01em] group-hover:text-[var(--tk-accent)]">{post.title}</h2>
          <p className="mt-2 line-clamp-2 text-[15px] leading-7 text-[var(--tk-muted)]">{getSummary(post) || 'Open this article to read more.'}</p>
        </div>
        <span className="hidden justify-self-end text-sm font-bold uppercase text-[var(--tk-accent)] sm:block">Read</span>
      </Link>
    </>
  )
}
...
function ListingArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <>
      {index === 0 ? (
        <div className="mx-auto w-full max-w-[300px] py-6">
          <Ads slot="sidebar" showLabel eager className="mx-auto min-h-[250px] w-full" />
        </div>
      ) : null}
